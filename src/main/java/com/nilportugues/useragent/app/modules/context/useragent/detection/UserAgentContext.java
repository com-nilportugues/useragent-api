...
class UserAgentContext {
    ...
    public static void test() {
        ...
        if (!context.getRegularTokens().equals("[ \"a\", \"b\", \"c\", \"g\", \"h\", \"i\"]"))
            throw new RuntimeException("Fail 1: " + context.getRegularTokens());
        if (!context.getParenTokens().equals("[ \"j\", \"k 2\", \"l\", \"d/3.3\", \"e\", \"f\"]"))
            throw new RuntimeException("Fail 2: " + context.getParenTokens());
        if (!String.valueOf(s = context.getcVersionAfterPattern("d/", MatchingType.BEGINS, MatchingRegion.REGULAR)).equals("null"))
            throw new RuntimeException("Fail 3: " + s);
        if (!String.valueOf(s = context.getcVersionAfterPattern("d/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS)).equals("3.3"))
            throw new RuntimeException("Fail 4: " + s + " " + context.getDebug());
        if (!String.valueOf(s = context.getcVersionAfterPattern("d/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS)).equals("null"))
            throw new RuntimeException("Fail 5: " + s);
        if (!context.getParenTokens().equals("[ \"j\", \"k 2\", \"l\", \"e\", \"f\"]"))
            throw new RuntimeException("Fail 6: " + context.getParenTokens());
        ...
        if (!context.getRegularTokens().equals("[ \"a\", \"b\", \"c\", \"i\"]"))
            throw new RuntimeException("Fail 8: " + context.getRegularTokens());
        ...
        if (!context.getRegularTokens().equals("[ \"a\", \"b\", \"c\", \"i\"]"))
            throw new RuntimeException("Fail 9: " + context.getRegularTokens());
        ...
        if (!context.getRegularTokens().equals("[ \"a\", \"b\", \"c\", \"i\"]"))
            throw new RuntimeException("Fail 11: " + context.getRegularTokens());

        context = new UserAgentContext(
            "Mozilla/5.0 (Linux; U; Android 4.0.4; fr-fr; GT-I9300-ORANGE/I9300BVBLH2 Build/IMM76D) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30");
        if (!context.getRegularTokens().equals("[ \"Mozilla/5.0\", \"AppleWebKit/534.30\", \"Version/4.0\", \"Mobile\", \"Safari/534.30\"]"))
            throw new RuntimeException("Fail 12: " + context.getRegularTokens());
        if (!context.getParenTokens().equals("[ \"KHTML, like Gecko\", \"Linux\", \"U\", \"Android 4.0.4\", \"fr-fr\", \"GT-I9300-ORANGE/I9300BVBLH2\", \"Build/IMM76D\"]"))
            throw new RuntimeException("Fail 13: " + context.getParenTokens());

        context = new UserAgentContext("Mozilla/5.0");
        if (!(s = context.getcRegion("Mozilla/([0-9\\.]+)", MatchingRegion.REGULAR, 1)).equals("5.0"))
            throw new RuntimeException("Fail 14: " + s);
    }
    ...
    private String[] getAndConsumeTokens(Matcher[] match, List<String> _tokens, List<String> consumeTo) {
        if (DEBUG)
            System.out.println("getAndConsumeTokens " + debugList(tokens) + " matching " + debugList(match));
        ...
        while (it.hasNext()) {
            ...
            if (matched) {
                ...
                it.remove();
                if (DEBUG)
                    System.out.println("_tokens is " + getRegularTokens());
                consumeTo.add(token);
                res[i++] = token;
                if (i == match.length) {
                    if (DEBUG)
                        System.out.println("return " + debugList(res));
                    return res;
                }
            } else {
                if (DEBUG)
                    System.out.println("not matched. Rewinding by " + i);
                if (i > 0) {
                    while (i > 0) {
                        it.previous();
                        it.add(res[--i]);
        ...
        if (i < match.length && i > 0) {
            if (DEBUG)
                System.out.println("end reached && not matched. Rewinding by " + i);
            boolean first = true;
            while (i > 0) {
                if (!first)
                    it.previous();
    ...
    public String getRemainingTokens() {
        if (tokens.isEmpty() && parenTokens.isEmpty())
            return "";
