package com.nilportugues.useragent.app.modules.context.useragent.detection;
...
public class UserAgentDetector implements IUserAgentDetector {
    ...
    static OS getOS(UserAgentContext context) {
        ...
        if (context.contains("Series40", MatchingType.BEGINS, MatchingRegion.PARENTHESIS)) {
            ...
            if (ver != null && ver.length() > res.getVersion().length()) // SonyEricsson UAs include the version twice. Get the most precise
                                                                         // one.
                res.setVersion(ver);
            ...
            if (context.consume("Windows NT 5.1", MatchingType.CONTAINS, MatchingRegion.PARENTHESIS)) {
                ...
                while (context.consume("uE v7", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                    ; // Don't really know what that is, but some XP UAs show it.
                boolean sp2 = false;
                ...
                if ((ver = context.getcVersionAfterPattern("Media Center PC ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS)) != null) {
                    if (ver.equals("2.7") || ver.equals("2.8"))
                        ...
                        if (ver.equals("3.1"))
                            res.appendToVersion(" (update rollup 1)");
                        if (ver.equals("4.0"))
                            res.appendToVersion(" (update rollup 2)");
                        if (!sp2) {
                            res.appendToVersion(" SP2");
                            sp2 = true;
                        }
                    ...
                    context.consume("Media Center PC ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS); // Sometimes present more than
                                                                                                          // once
                }
                ...
                if (context.contains("Windows NT 5.01", MatchingType.BEGINS, MatchingRegion.CONSUMED))
                    res.appendToVersion(" SP1");
            ...
            if (ver == null)
                ver = "";
            if (ver.length() > 0) {
                if (!ver.matches("[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+"))
                    ver = "";
            }
    ...
    static RenderingEngine getKHTMLVersion(UserAgentContext context) {
        try {
            ...
            if (ver != null)
                return new RenderingEngine(Brand.OPENSOURCE_COMMUNITY, RenderingEngineFamily.KHTML, ver, 2);
            ...
            if (ver != null)
                return new RenderingEngine(Brand.OPENSOURCE_COMMUNITY, RenderingEngineFamily.KHTML, "for Konqueror " + ver, 2);
    ...
    static RenderingEngine getTridentVersion(UserAgentContext context, String ieWithVersion) {
        ...
        if (tver != null)
            return new RenderingEngine(Brand.MICROSOFT, RenderingEngineFamily.TRIDENT, tver, 2);
    ...
    static RenderingEngine getWebkitVersion(UserAgentContext context, String chromeVersion, boolean couldBeBlink, boolean isBlink) {
        try {
            ...
            if (ver != null)
                return new RenderingEngine(brand, refam, ver, 2);
    ...
    static Browser getGecko(UserAgentContext context, String ver, OS os) {
        ...
        for (Map.Entry<String, GeckoSpinoff> so : geckoSpinOffs.entrySet()) {
            if ((ver = context.getcVersionAfterPattern(so.getKey() + "/", MatchingType.BEGINS, MatchingRegion.REGULAR)) != null) {
                ...
                if (so.getValue().getBrand() != null)
                    res.setVendor(so.getValue().getBrand());
        ...
        if (found) {
            ...
            context.consume("Fennec/", MatchingType.BEGINS, MatchingRegion.REGULAR); // This is FF for Android, but we already have the OS
                                                                                     // to tell us that.
            res.setFullVersionOneShot(ver, 2);
            res.setDescription("Firefox");
            res.setFamily(BrowserFamily.FIREFOX);
    ...
    static Browser tryGetIE(UserAgentContext context, String possibleVersions, OS os) {
        ...
        if ((verie = context.getcVersionAfterPattern("MSIE ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS)) != null) {
            ...
            if ((vertr = context.getcVersionAfterPattern("Trident/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS)) != null ||
                ...
                res = new Browser(Brand.MICROSOFT, BrowserFamily.IE, "IE",
                    new RenderingEngine(Brand.MICROSOFT, RenderingEngineFamily.TRIDENT, "for IE " + StringUtils.format(iever), 2), verie);
            }
            ...
            if (possibleVersions.indexOf(String.valueOf((int) Math.floor(iever)) + ",") == -1)
                res = null;
    ...
    static Browser getBrowser(UserAgentContext context, OS os, OS[] overrideOS) {
        ...
        if ((ver = context.getcVersionAfterPattern("SMIT-Browser/", MatchingType.BEGINS, MatchingRegion.REGULAR)) != null) {
            ...
            context.getcNextTokens(
                new Matcher[] {new Matcher("Windows", MatchingType.EQUALS), new Matcher("Phone", MatchingType.EQUALS), new Matcher("Search", MatchingType.EQUALS)},
                MatchingRegion.REGULAR);
            ...
            if ((ver = context.getcVersionAfterPattern("NokiaBrowser/", MatchingType.BEGINS, MatchingRegion.REGULAR)) != null) {
                ...
                if (os.getDescription().contains("Android")) {
                    if (!UserAgentDetectionHelper.greaterThan(os.getVersion(), 4)
                        && (context.consume("Version/", MatchingType.BEGINS, MatchingRegion.REGULAR) || app.length() > 0)) {
                ...
                if (context.consume("Alpha", MatchingType.EQUALS, MatchingRegion.REGULAR))
                    ver += " alpha";
                ...
                if (os.getVendor() == Brand.SONY) {
                    res = new Browser(Brand.SONY, BrowserFamily.OTHER_WEBKIT, "NetFront fork", getWebkitVersion(context), ver); // According
                                                                                                                                // to
                                                                                                                                // http://console.maban.co.uk/device/psvita/
                } else {
            ...
            ((ver = context.getcVersionAfterPattern("ELinks/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS)) != null
                && context.consume("Mozilla/5.0", MatchingType.EQUALS, MatchingRegion.REGULAR))) {
            res = new Browser(Brand.UNKNOWN, BrowserFamily.TEXTBASED, "ELinks", RenderingEngine.getText(), ver);
            ...
            res = new Browser(Brand.OPERA, BrowserFamily.OPERA, "Opera For DSi", new RenderingEngine(Brand.OPERA, RenderingEngineFamily.PRESTO, "2.1", 2), ver); // See
                                                                                                                                                                 // http://en.wikipedia.org/wiki/Nintendo_DS_%26_DSi_Browser
        ...
        if (context.contains("Mozilla/4\\.[01]", MatchingType.REGEXP, MatchingRegion.REGULAR) &&
            ...
            if (res == null)
                res = tryGetIE(context, "4,5,6,7,8,", os);
            if (res != null) {
                context.consume("Mozilla/4.0", MatchingType.EQUALS, MatchingRegion.REGULAR);
                context.consume("compatible", MatchingType.EQUALS, MatchingRegion.PARENTHESIS);
            }
    ...
    static Device getDevice(UserAgentContext context, Browser b, OS o) {
        ...
        // Nokia stuff
        // if (context.getcToken("Nokia7650/", MatchingType.BEGINS, MatchingRegion.BOTH) != null) return new
        // Device("",DeviceType.BOT,Brand.UNKNOWN,"COUCOUCOUCOU");
        if (o.getVendor() == Brand.NOKIA) {
            Device res = new Device(arm, DeviceType.PHONE, Brand.NOKIA, "");
            if (context.getcToken("NokiaN95", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                res.setDevice("N95");
            if (context.getcToken("NokiaN9", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                res.setDevice("N9");
            if (o.getDescription().equals("Series40") || o.getDescription().equals("Symbian OS")) {
                if (context.getcToken("Nokia311", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("Asha 311");
                if (context.getcToken("NokiaX3-02", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("X3 Touch and Type");
                if (context.getcToken("Nokia305", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("Asha 305");
                if (context.getcToken("NokiaC3-00", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("C3-00");
                if (context.getcToken("NokiaC7-00", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("C7-00");
                if (context.getcToken("Nokia202", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("Asha 202");
                if (context.getcToken("Nokia 3650", MatchingType.EQUALS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("3650");
                if (context.getcToken("Nokia6300", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("6300");
                if (context.getcToken("Nokia7650/", MatchingType.BEGINS, MatchingRegion.REGULAR) != null)
                    res.setDevice("7650");
                if (context.getcToken("NokiaX2-02", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("X2-02");
                if (context.getcToken("NokiaX6-00", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("X6-00");
                if (context.getcToken("NokiaN73", MatchingType.BEGINS, MatchingRegion.BOTH) != null)
                    res.setDevice("N73");
                if (context.getcToken("NokiaN8-", MatchingType.BEGINS, MatchingRegion.BOTH) != null)
                    res.setDevice("N8");
                if (context.getcToken("NokiaN81-", MatchingType.BEGINS, MatchingRegion.BOTH) != null)
                    res.setDevice("N81");
                if (context.getcToken("NokiaE5-", MatchingType.BEGINS, MatchingRegion.BOTH) != null)
                    res.setDevice("E5");
                if (context.getcToken("NokiaE51-", MatchingType.BEGINS, MatchingRegion.BOTH) != null)
                    res.setDevice("E51");
                if (context.getcToken("NokiaE63-", MatchingType.BEGINS, MatchingRegion.BOTH) != null)
                    res.setDevice("E63");
                if (context.getcToken("NokiaE71-", MatchingType.BEGINS, MatchingRegion.BOTH) != null)
                    res.setDevice("E71");
                if (context.getcToken("NokiaE60", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("E60");
                if (context.getcToken("NokiaE90", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("E90");
                if (context.getcToken("NokiaN85-", MatchingType.BEGINS, MatchingRegion.BOTH) != null)
                    res.setDevice("N85");
                if (context.getcToken("NokiaN86-", MatchingType.BEGINS, MatchingRegion.BOTH) != null)
                    res.setDevice("N86");
                if (context.getcToken("NokiaN72/", MatchingType.BEGINS, MatchingRegion.BOTH) != null)
                    res.setDevice("N72");
                if (context.getcToken("NokiaN93-", MatchingType.BEGINS, MatchingRegion.BOTH) != null)
                    res.setDevice("N93");

                if (context.getcToken("Nokia808PureView/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("808 PureView");
                if (context.getcToken("Nokia5233/", MatchingType.BEGINS, MatchingRegion.BOTH) != null)
                    res.setDevice("5233");
                if (context.getcToken("Nokia5230/", MatchingType.BEGINS, MatchingRegion.BOTH) != null)
                    res.setDevice("5230");
                if (context.getcToken("Nokia500/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("500");
                if (context.getcToken("Nokia701", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("701");
                if (context.getcToken("Nokia3230", MatchingType.BEGINS, MatchingRegion.REGULAR) != null)
                    res.setDevice("3230");
                if (context.getcToken("es50", MatchingType.EQUALS, MatchingRegion.REGULAR) != null)
                    res.setDevice("ES50");
                if (context.getcToken("es61", MatchingType.EQUALS, MatchingRegion.REGULAR) != null)
                    res.setDevice("ES61");
                if (context.getcToken("es61i", MatchingType.EQUALS, MatchingRegion.REGULAR) != null)
                    res.setDevice("ES61");
                if (context.getcToken("Nokia[ ]?6630/.*", MatchingType.REGEXP, MatchingRegion.BOTH) != null)
                    res.setDevice("6630");
                if (context.getcToken("Nokia5530c-2/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("5530 XpressMusic");
                if (context.getcToken("Nokia5800d", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) != null)
                    res.setDevice("5800 XpressMusic");
            }
            ...
            if (context.getUA().startsWith("SonyEricsson")) {
                ...
                if (context.consume("SonyEricssonU5", MatchingType.BEGINS, MatchingRegion.REGULAR))
                    res.setDevice("Vivaz");
                if (context.consume("SonyEricssonU1[ai]/.*", MatchingType.REGEXP, MatchingRegion.REGULAR))
                    res.setDevice("Satio");
                if (res.getDevice().length() == 0) {
        ...
        if (o.getFamily() == OSFamily.WEBOS) {
            ...
            if (context.consume("/Palm 500v/", MatchingType.BEGINS, MatchingRegion.REGULAR))
                return new Device(arm, DeviceType.PHONE, Brand.PALM, "Treo 500v", true);
            if (context.consume("PalmSource/Palm-D052", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
            ...
            if (context.consume("Palm750/v0000", MatchingType.BEGINS, MatchingRegion.REGULAR))
                return new Device(arm, DeviceType.PHONE, Brand.PALM, "Treo 750w", true);
            if (context.consume("PalmSource/Palm-D060", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.PALM, "Treo 755p", true);
            if (context.consume("Treo800w/v0100", MatchingType.BEGINS, MatchingRegion.REGULAR))
                return new Device(arm, DeviceType.PHONE, Brand.PALM, "Treo 800w", true);
            if (context.consume("Treo850/v0100", MatchingType.BEGINS, MatchingRegion.REGULAR))
                return new Device(arm, DeviceType.PHONE, Brand.PALM, "Treo850", true);
            if (context.consume("Alltel_Treo850e", MatchingType.BEGINS, MatchingRegion.REGULAR))
                return new Device(arm, DeviceType.PHONE, Brand.PALM, "Treo850e", true);
            if (context.consume("PalmSource/Palm-TnT5", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.PALM, "Treo 600", true);
            if (context.consume("PalmSource/Palm-TunX", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.PALM, "Tunx", true);
            if (context.consume("PalmSource/hspr-H102", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.PALM, "Treo 650", true);
            if (context.consume("PalmSource/Palm-D050", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.PALM, "TX", true);
            if (context.consume("PalmSource/Palm-D062", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.PALM, "Centro", true);
            if (context.consume("PalmSource/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
        ...
        if (o.getFamily() == OSFamily.ANDROID) {
            ...
            if (context.contains("SAMSUNG[- ]SGH-.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS) ||
                ...
                Device device = null;
                if (context.consume("(SAMSUNG-)?SGH-T999.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3");
                else if (context.consume("(SAMSUNG-)?SGH-T989.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S2");
                else if (context.consume("(SAMSUNG-)?SGH-T959.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S");
                else if (context.consume("(SAMSUNG-)?SGH-T889.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note 2");
                else if (context.consume("(SAMSUNG-)?SGH-T769.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S Blaze 4G");
                else if (context.consume("(SAMSUNG-)?SGH-T759.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Exhibit 4G");
                else if (context.consume("(SAMSUNG-)?SGH-I896.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S");
                else if (context.consume("(SAMSUNG-)?SGH-I747.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3");
                else if (context.consume("(SAMSUNG-)?SGH-I337.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S4");
                else if (context.consume("(SAMSUNG )?SGH-M919.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S4");
                else if (context.consume("(SAMSUNG-)?SGH-I407.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Amp");
                else if (context.consume("(SAMSUNG-)?SGH-I897.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Captivate");
                else if (context.consume("(SAMSUNG-)?SGH-I7[27]7.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S2");
                else if (context.consume("(SAMSUNG-)?SGH-I717.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note");
                else if (context.consume("(SAMSUNG-)?SGH-I317.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note 2");
                else if (context.consume("(SAMSUNG-)?SGH-T679.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Exhibit II 4G");
                else if (context.consume("(SAMSUNG-)?SGH-I997.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Infuse 4G");
                else if (context.consume("(SAMSUNG )?SGH-T849.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 7.0");
                else if (context.consume("(SAMSUNG )?SGH-T859.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 10.1");
                else if (context.consume("(SAMSUNG )?SGH-T499.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Dart");
                else if (context.consume("(SAMSUNG )?SGH-T589.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Gravity Smart");
                else if (context.consume("(SAMSUNG )?SGH-T839.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Sidekick 4G");
                else if (context.consume("(SAMSUNG )?SGH-T399.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Light");
            ...
            if (context.contains("SAMSUNG GT-", MatchingType.BEGINS, MatchingRegion.PARENTHESIS) ||
                ...
                context.consume("Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS);
                if (context.consume("(SAMSUNG )?GT-I9100.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S2", true);
                if (context.consume("(SAMSUNG )?GT-S6102.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Y Duos", true);
                if (context.consume("(SAMSUNG )?GT-S5839i.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Ace", true);
                if (context.consume("(SAMSUNG )?GT-S5830.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Ace", true);
                if (context.consume("(SAMSUNG )?GT-S5690.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Xcover", true);
                if (context.consume("(SAMSUNG )?GT-S5670.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Fit", true);
                if (context.consume("(SAMSUNG )?GT-S5660.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Gio", true);
                if (context.consume("(SAMSUNG )?GT-S536[0-3].*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Y", true);
                if (context.consume("(SAMSUNG )?GT-S5570.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Mini", true);
                if (context.consume("(SAMSUNG )?GT-S7275R.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Ace 3", true);
                if (context.consume("(SAMSUNG )?GT-S7562.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S Duos", true);

                if (context.consume("(SAMSUNG )?GT-P75[01]0.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 10.1", true);
                if (context.consume("(SAMSUNG )?GT-P68[01]0.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 7.7", true);
                if (context.consume("(SAMSUNG )?GT-P73[01]0.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 8.9", true);
                if (context.consume("(SAMSUNG )?GT-P6200.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 7 Plus", true);
                if (context.consume("(SAMSUNG )?GT-P51[01]0.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 2 10.1", true);
                if (context.consume("(SAMSUNG )?GT-P5113.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 2 10.1", true);
                if (context.consume("(SAMSUNG )?GT-P5210.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 3 10.1", true);
                if (context.consume("(SAMSUNG )?GT-P3113.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 2 7", true);
                if (context.consume("(SAMSUNG )?GT-P31[01]0.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 2 7", true);
                if (context.consume("(SAMSUNG )?GT-P10[01]0.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab", true);
                if (context.consume("(SAMSUNG )?GT-P6210.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab", true);

                if (context.consume("(SAMSUNG )?GT-N80[01]0.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Note 10.1", true);
                if (context.consume("(SAMSUNG )?GT-N8005.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Note 10.1", true);
                if (context.consume("(SAMSUNG )?GT-N8013.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note", true);
                if (context.consume("(SAMSUNG )?GT-N5110.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Note 8.0", true);
                if (context.consume("(SAMSUNG )?GT-N7105.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note 2", true);
                if (context.consume("(SAMSUNG )?GT-N7100.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note 2", true);
                if (context.consume("(SAMSUNG )?GT-N7100.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note 2", true);
                if (context.consume("(SAMSUNG )?GT-N7000.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note", true);
                // TODO: UNTESTED
                if (context.consume("(SAMSUNG )?GT-5100.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Note 8.0");

                if (context.consume("(SAMSUNG )?GT-I5510.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy 551", true);
                if (context.consume("(SAMSUNG )?GT-I8190.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3 Mini", true);
                if (context.consume("(SAMSUNG )?GT-I5500.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy 5", true);
                if (context.consume("(SAMSUNG )?GT-I9001.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S Plus", true);
                if (context.consume("(SAMSUNG )?GT-I5700.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Spica", true);
                if (context.consume("(SAMSUNG )?GT-I9305T.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3", true);
                if (context.consume("(SAMSUNG )?GT-I930[05].*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3", true);
                if (context.consume("(SAMSUNG )?GT-I950[056].*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S4", true);
                if (context.consume("(SAMSUNG )?GT-I9515.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S4 Value Edition", true);
                if (context.consume("(SAMSUNG )?GT-I9220.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note", true);
                if (context.consume("(SAMSUNG )?GT-I9295.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S4 Active", true);
                if (context.consume("(SAMSUNG )?GT-I919[05].*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S4 Mini", true);
                if (context.consume("(SAMSUNG )?GT-I9000.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S", true);
                if (context.consume("(SAMSUNG )?GT-I9003.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy SL", true);
                if (context.consume("(SAMSUNG )?GT-I8160.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Ace 2", true);
                if (context.consume("(SAMSUNG )?GT-I5800.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy 3", true);
                if (context.consume("(SAMSUNG )?GT-I5801.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Apollo", true);
                if (context.consume("(SAMSUNG )?GT-I8150.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy W", true);

                if (context.consume("(SAMSUNG )?GT-B5510.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Y Pro", true);
                if (context.consume("(SAMSUNG )?GT-B7510.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Pro", true);

                if (context.consume("(SAMSUNG )?GT-S7262.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Star Pro/Plus", true);

                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Unknown", true);
            }

            if (context.consume("Galaxy S II", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S2", true);
            if (context.consume("SHV-E120K ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S2", true);
            if (context.consume("Galaxy Build", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
            ...
            if (context.consume("SCH-R530U", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3", true);
            if (context.consume("SCH-I939", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3", true);
            if (context.consume("SCH-I605", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note 2", true);
            if (context.consume("SCH-I535", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3", true);
            if (context.consume("SCH-I545", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S4", true);
            if (context.consume("SCH-I500", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S", true);
            if (context.consume("SC-06D", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3", true);
            if (context.consume("SPH-M820", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Prevail", true);
            if (context.consume("SPH-L900", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note 2", true);
            if (context.consume("SPH-L710", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3", true);
            if (context.consume("(SAMSUNG )?SPH-L720T .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S4", true);
            if (context.consume("SPH-D710", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S2", true);
            if (context.consume("SPH-M920", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Transform", true);
            if (context.consume("SPH-M900", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Moment", true);
            if (context.consume("SPH-D700", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Epic 4G", true);
            if (context.consume("SPH-M910", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Intercept", true);
            if (context.consume("SPH-M930", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Transform Ultra", true);
            if (context.consume("SPH-P100", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 7", true);
            if (context.consume("SPH-D600", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Conquer 4G", true);

            if (context.consume("SHW-M380[KW] .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 10.1", true);
            if (context.consume("SHW-M250K ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S2", true);
            if (context.consume("SHW-M110S ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S", true);
            if (context.consume("SHW-M440S ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3", true);
            if (context.consume("SHV-E210S ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3", true);
            if (context.consume("SHV-E210L ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3", true);
            if (context.consume("SHV-E210K ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S3", true);
            if (context.consume("SHV-E160[SK] .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note", true);
            if (context.consume("SM-N7505 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note 3 Neo", true);
            if (context.consume("(SAMSUNG[ -])?SM-N900[AVSPT56]?(-ORANGE)?[ /].*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note 3", true);
            if (context.consume("(SAMSUNG[ -])?SM-N900(W8|0Q)(-ORANGE)?[ /].*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note 3", true);
            if (context.consume("(SAMSUNG[ -])?SM-G870A[ /].*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S5 Active", true);
            if (context.consume("(SAMSUNG[ -])?SM-G90[01][VATF][ -].*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S5", true);
            if (context.consume("(SAMSUNG[ -])?SM-G350(2T)? .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Core Plus", true);
            if (context.consume("(SAMSUNG[ -])?SM-T21(1|7S|7A|0R|0|05) .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 3 7.0", true);
            if (context.consume("SM-T230 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 4 7.0", true);
            if (context.consume("SM-T310 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 3 8.0", true);
            if (context.consume("SM-T805 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab S 10.5", true);
            if (context.consume("(SAMSUNG[ -])?SM-T320 .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab Pro 8.4", true);
            if (context.consume("(SAMSUNG[ -])?SM-T550 .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab A 9.7", true);
            if (context.consume("(SAMSUNG[ -])?SM-T900 .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab Pro 12.2", true);
            if (context.consume("SM-T520 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab Pro 10.1", true);
            if (context.consume("SM-T530 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab 4 10.1", true);
            if (context.consume("(SAMSUNG[ -])?SM-T800 .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Tab S 10.5", true);
            if (context.consume("SM-G800F ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S5 Mini", true);
            if (context.consume("SM-P600 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Note 10.1", true);
            if (context.consume("(SAMSUNG[ -])?SM-P900 .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SAMSUNG, "Galaxy Note Pro 12.2", true);
            if (context.consume("(SAMSUNG[ -])?SM-N910([TGSAUHVFCP]|W8)(-ORANGE| |/[A-Z0-9]+).*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note 4", true);
            if (context.consume("(SAMSUNG[ -])?(SM-)?N9100 .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Note 4 Dual Sim", true);
            if (context.consume("SM-G386F ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Core", true);
            if (context.consume("SM-G850M ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Alpha", true);
            if (context.consume("SM-A300FU ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy A3", true);
            if (context.consume("(SAMSUNG )?SM-G850F.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Alpha", true);
            if (context.consume("(SAMSUNG[ -])?SM-G920([TAF]|W8)(-ORANGE| |/[A-Z0-9]+).*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S6", true);
            if (context.consume("(SAMSUNG[ -])?SM-G357FZ.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy Ace 4", true);
            if (context.consume("(SAMSUNG[ -])?SM-G950[UF].*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Galaxy S8", true);

            // KTTECH
            if (context.consume("KM-E100", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.KTTECH, "KM-E100", true);

            // ZTE
            if (context.consume("ZTE-N880E", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ZTE, "N880E", true);
            if (context.consume("ZTE U970_TD/1.0", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ZTE, "Grand X", true);
            if (context.consume("ZTE N880E", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ZTE, "N880E", true);
            if (context.consume("ZTE Z992", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ZTE, "Z992", true);
            if (context.consume("Z995", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ZTE, "Z995", true);
            if (context.consume("Z730", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ZTE, "Concord II", true);
            if (context.consume("Orange Tactile internet 2", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ZTE, "Blade", true);
            if (context.consume("ZTE V768", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ZTE, "Concord (V768)", true);
            if (context.consume("ZTE-RACER", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ZTE, "Racer", true);
            if (context.consume("ZTE-BLADE", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ZTE, "Blade", true);
            if (context.consume("ZTE-U V880", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ZTE, "Blade V880", true);
            if (context.consume("ZTE-Z667G", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ZTE, "Whirl 2", true);

            // Huawei
            if (context.consume("U8220", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Pulse U8220", true);
            if (context.consume("U8350", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Boulder U8350", true);
            if (context.consume("Huawei U8800", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Ideos X5", true);
            if (context.consume("U8180", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Boulder Ideos X1", true);
            if (context.consume("HUAWEI-M835", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Ideos", true);
            if (context.consume("HUAWEI-M860", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Ascend", true);
            if (context.consume("H866C", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Ascend Y", true);
            if (context.consume("HUAWEI_T8620_", MatchingType.BEGINS, MatchingRegion.REGULAR))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Ascend Y200T (T8620)", true);
            if (context.consume("T-Mobile myTouch Build/HuaweiU8680", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "myTouch (8680)", true);
            if (context.consume("Prism Build/HuaweiU8651", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Prism (8651)", true);
            if (context.consume("U8815", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Ascend G300 (U8815)", true);
            if (context.consume("HUAWEI U8950", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Ascend G600 (U8950)", true);
            if (context.consume("Huawei-U8665", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Fusion 2", true);
            if (context.consume("Huawei-U8652", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Fusion U8652", true);
            if (context.consume("TURKCELL MaxiPRO5", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Honor U8860", true);
            if (context.consume("HUAWEI MT7-L09 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Ascend Mate 7", true);
            if (context.consume("H60-L04 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Honor 6", true);
            if (context.consume("U8650 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HUAWEI, "Sonic", true);

            // SONY
            if (context.consume("SonyST23i ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Miro", true);
            if (context.consume("SonyST21i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia", true);
            if (context.consume("(SonyEricsson)?U20i.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia X10 mini pro", true);
            if (context.consume("SonyLT30p", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia T", true);
            if (context.consume("Sony Tablet S", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SONY, "Tablet S", true);
            if (context.consume("SonyEricssonLT22i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia P", true);
            if (context.consume("SonyEricssonLT15a", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Arc", true);
            if (context.consume("SonyEricssonE10i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia X10 Mini", true);
            if (context.consume("SonyEricssonU20a", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia X10 Mini Pro", true);
            if (context.consume("SonyEricssonMT15i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Neo", true);
            if (context.consume("SonyEricssonMT11i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Neo V", true);
            if (context.consume("SonyEricssonWT19i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia E", true);
            if (context.consume("SonyEricssonR800i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Play", true);
            if (context.consume("SonyEricssonST27i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Go", true);
            if (context.consume("SonyEricssonE10a", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia X10 Mini", true);
            if (context.consume("SonyEricssonX10i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia X10", true);
            if (context.consume("SonyEricssonX10a", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia X10a", true);
            if (context.consume("SonyEricssonSO-01B", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia X10", true);
            if (context.consume("SonyEricssonR800a", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Play 4G", true);
            if (context.consume("SonyEricssonR800x", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Play", true);
            if (context.consume("SonyEricssonE15a", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia X8", true);
            if (context.consume("SonyEricssonE15i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia X8", true);
            if (context.consume("SonyEricssonMK16i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Pro", true);
            if (context.consume("SonyEricssonLT15i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Arc", true);
            if (context.consume("SonyEricssonST18i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Ray", true);
            if (context.consume("SonyEricssonST18a", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Ray", true);
            if (context.consume("SonyEricssonSK17i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Mini Pro", true);
            if (context.consume("SonyEricssonLT26i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia S", true);
            if (context.consume("SonyEricssonST25i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia U", true);
            if (context.consume("SonyEricssonLT18i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Arc S", true);
            if (context.consume("SO-01C", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia arc", true);
            if (context.consume("SGP321", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.SONY, "Xperia Tablet Z", true);
            if (context.consume("C6603", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Z", true);

            if (context.consume("LT26i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia S", true);
            if (context.consume("LT26i", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia S", true);
            if (context.consume("SGP611 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Z3", true);
            if (context.consume("D6503 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Z2", true);
            if (context.consume("C6903 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Z1", true);
            if (context.consume("D5503 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Z1", true);
            if (context.consume("C1904 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia M", true);
            if (context.consume("C6833 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Z Ultra", true);
            if (context.consume("C5303 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia SP", true);
            if (context.consume("D58[03]3 .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Z3 Compact", true);
            if (context.consume("D66(03|16|43|53) .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Z3", true);
            if (context.consume("D6633 .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SONY, "Xperia Z3 Dual", true);

            // Sharp
            if (context.consume("SBM106SH", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.SHARP, "SBM106SH", true);

            // HTC
            if (context.consume("Sprint APX515CKT", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "Evo 3D X515xkt", true);
            if (context.consume("Sprint APA9292KT", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "Evo 4G", true);
            if (context.consume("Sprint APA7373KT", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "EVO Shift 4G", true);
            if (context.consume("EVO ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "Evo 4G", true);
            if (context.consume("HTC Inspire 4G ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "Inspire 4G", true);
            if (context.consume("HTC_Runnymede ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "Runnymede", true);

            if (context.consume("ADR6300 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "Droid Incredible", true);
            if (context.consume("(USCC)?ADR6325(US)? .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "Merge", true);
            if (context.consume("pcdadr6350 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "ADR6350", true);
            if (context.consume("PC36100 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "One XL", true);
            if (context.consume("IncredibleS_S710e", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "Incredible S", true);
            if (context.consume("Incredible S ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "Incredible S", true);
            if (context.consume("HTL21", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "J Butterfly", true);
            if (context.consume("(HTC[ _])?EVO( )?3D[ _]X515m.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "Evo 3D X515m", true);
            if (context.consume("(HTC_)?Amaze_4G.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "Amaze 4G", true);
            if (context.getUA().indexOf("HTC") > 0) {
                if (context.consume("HTC_Flyer_P512_NA ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Flyer", true);
                if (context.consume("HTC[_ ]Dream .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Dream", true);
                if (context.consume("HTC_S510b ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Rhyme", true);
                if (context.consume("HTC Liberty ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Liberty/Aria/Intruder/A6366", true);
                if (context.consume("HTC_WildfireS", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Wildfire S (Marvel)", true);
                if (context.consume("HTCA510e/1.0", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "WildFire", true);
                if (context.consume("HTCA510e", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Wildfire (Buzz)", true);
                if (context.consume("HTC[_ ]Sensation[_ ]4G.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Sensation 4G", true);
                if (context.consume("HTC_PH39100/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Vivid", true);
                if (context.consume("HTC_T120C ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "One V", true);
                if (context.consume("HTC_One_X", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "One X", true);
                if (context.consume("HTC EVA_UL", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "One X Evita", true);
                if (context.consume("HTC_One_S", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "One S", true);
                if (context.consume("HTC_DesireS_S510e", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire S", true);
                if (context.consume("HTC/DesireS/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire S", true);
                if (context.consume("HTC_DesireHD_A9191", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire HD", true);
                if (context.consume("HTC_C715c", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "EVO Design 4G", true);
                if (context.consume("HTC Sensation Z710e", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Sensation Z710e", true);
                if (context.consume("HTC Glacier", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Glacier", true);
                if (context.consume("HTC_Rhyme_S510b", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Rhyme", true);
                if (context.consume("HTC/WildfireS/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Wildfire S", true);
                if (context.consume("HTC-PG762 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Wildfire S", true);
                if (context.consume("HTC/Sensation/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Sensation", true);
                if (context.consume("HTC_SensationXL_Beats-", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Sensation XL Beats", true);
                if (context.consume("HTC One X", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "One X", true);
                if (context.consume("HTC Click-", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Click/Tattoo", true);
                if (context.consume("HTC Incredible S", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Incredible S", true);
                if (context.consume("HTC Desire HD", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire HD", true);
                if (context.consume("HTC HD2", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "HD2/Leo", true);
                if (context.consume("HTC[_ ]Wildfire[-_ ].*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Wildfire", true);
                if (context.consume("HTC Desire C", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire C", true);
                if (context.consume("HTC_DesireZ_", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire Z", true);
                if (context.consume("HTC-A7275", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire Z", true);
                if (context.consume("HTC D816v ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire 816", true);
                if (context.consume("HTC D816h ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire 816", true);
                if (context.consume("HTC Desire 816", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire 816", true);
                if (context.consume("HTC Desire ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire", true);
                if (context.consume("HTC_Desire", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire", true);
                if (context.consume("HTC Hero ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Hero", true);
                if (context.consume("HTC Vision ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire Z", true);
                if (context.consume("HTC Legend ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Legend", true);
                if (context.consume("HTC One S ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "One S", true);
                if (context.consume("HTC[_ ]One[_ ]V .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "One V", true);
                if (context.consume("HTC Bravo ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Desire", true);
                if (context.consume("HTC Salsa C510b ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "Salsa", true);
                if (context.consume("HTC One Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "One", true);
                if (context.consume("HTCONE Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "One", true);
                if (context.consume("HTC_One Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.HTC, "One", true);
            ...
            if (context.consume("ADR6350 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
            ...
            if (context.consume("HTC Magic ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.HTC, "Magic", true);
            // LG
            {
                Device res = null;
                if (context.consume("LG-MS770 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Motion 4G");
                if (context.consume("Optimus 2X", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus 2X");
                if (context.consume("LGMS323 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus L70");
                if (context.consume("LG-V500 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.TABLET, Brand.LG, "G Pad 8.3");
                if (context.consume("Vortex ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Vortex");
                if (context.consume("LG-L[GS]855.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Marquee");
                if (context.consume("LG-LU6500", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus Q2");
                if (context.consume("LGL55C Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus Q");
                if (context.consume("LG-LS840 Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Viper");
                if (context.consume("LG-MS690 Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus M");
                if (context.consume("LG-MS695 Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus M+");

                if (context.consume("LG-P350", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus Me");
                if (context.consume("LG-P500", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus One");
                if (context.consume("LG-P509", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus T");
                if (context.consume("LG-P700", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus L7");
                if (context.consume("LG-P715", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus L7 II");
                if (context.consume("LG-P870", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Escape");
                if (context.consume("LG-P880", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus 4X HD");
                if (context.consume("LG-P920", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus 3D");
                if (context.consume("LG-P925", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Thrill 4G");
                if (context.consume("LG-P970", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus Black");
                if (context.consume("LG-P990", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus 2X");
                if (context.consume("LG-D618", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "G2 Mini");
                if (context.consume("LG-P999", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "G2x");
                if (context.consume("LG-US670", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus U");
                if (context.consume("LG-GT540", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus GT540");
                if (context.consume("LG-E400", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus L3");
                if (context.consume("LG-LS720", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus F3");
                if (context.consume("LG-E739", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "myTouch");
                if (context.consume("LG-C800 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "myTouch Q");
                if (context.consume("LG-VS700", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Enlighten");
                if (context.consume("LG-VM696", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus Elite");
                if (context.consume("VS415PP", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Optimus Zone 2");
                if (context.consume("LG Eve", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "Eve");
                if (context.consume("VS980 4G ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "G2");
                if (context.consume("LG-VS980 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                    res = new Device(arm, DeviceType.PHONE, Brand.LG, "G2");
            ...
            // Google branded
            if (context.consume("Galaxy Nexus", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.GOOGLE, Brand.SAMSUNG, "Galaxy Nexus", true);
            if (context.consume("Nexus S", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.GOOGLE, Brand.SAMSUNG, "Nexus S", true);
            if (context.consume("Nexus One", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.GOOGLE, Brand.HTC, "Nexus One", true);
            if (context.consume("Nexus 7", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.GOOGLE, Brand.ASUS, "Nexus 7", true);
            if (context.consume("Nexus 4", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.GOOGLE, Brand.LG, "Nexus 4", true);
            if (context.consume("Nexus 5", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.GOOGLE, Brand.LG, "Nexus 5", true);
            if (context.consume("Nexus 9", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.GOOGLE, Brand.HTC, "Nexus 10", true);
            if (context.consume("Nexus 10", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
            ...
            if (context.consume("MZ60[14].*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.MOTOROLA, "Xoom", true);
            if (context.consume("MB865 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Atrix 2", true);
            if (context.consume("MB860 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
            ...
            if (context.consume("MB526 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "DEFY+", true);

            if (context.consume("DROID2", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Droid 2", true);
            if (context.consume("DROID3", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Droid 3", true);
            if (context.consume("DROID4 4G", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Droid 4", true);
            if (context.consume("DROID RAZR 4G", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Droid Razr 4G", true);
            if (context.consume("DROID RAZR HD", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Droid Razr HD", true);
            if (context.consume("DROID RAZR Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Droid Razr", true);
            if (context.consume("Droid", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Droid", true);
            if (context.consume("DROID BIONIC 4G", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Droid Bionic 4G", true);
            if (context.consume("DROID BIONIC", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Droid Bionic", true);
            if (context.consume("DROID P[Rr][Oo] .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Droid Pro", true);
            if (context.consume("DROIDX ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Droid X", true);
            if (context.consume("DROID X2 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Droid X2", true);
            if (context.consume("MOTWX435KT", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Triumph", true);
            if (context.consume("XT1254 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Droid Turbo (Quark)", true);
            if (context.consume("XT1058 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Moto X", true);
            if (context.consume("XT1097 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Moto X XT1097", true);
            if (context.consume("XT890 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(atom, DeviceType.PHONE, Brand.MOTOROLA, "RAZR i", true);
            if (context.consume("XT321 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "Defy Mini", true);

            // Asus
            if (context.consume("ASUS Transformer Pad TF700T", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.ASUS, "Transformer Pad Infinity", true);
            if (context.consume("ASUS Transformer Pad TF300T", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.ASUS, "Transformer Pad TF300T", true);
            if (context.consume("Transformer TF101", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.ASUS, "Eee Pad Transformer", true);
            if (context.consume("Transformer Prime TF201", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.ASUS, "Eee Pad Transformer Prime", true);
            if (context.consume("K014 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device("Intel Bay Trail", DeviceType.TABLET, Brand.ASUS, "K014", true);
            if (context.consume("ME173X ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.ASUS, "MeMO Pad HD7", true);

            // Acer
            if (context.consume("A700", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.ACER, "Iconia Tab A700", true);
            if (context.consume("A1-810", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.ACER, "Iconia A1-810", true);
            if (context.consume("A1-840", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.ACER, "Iconia Tab 8", true);

            // Lenovo
            if (context.consume("Lenovo P700", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.LENOVO, "P700", true);
            if (context.consume("(Lenovo )?K900 .*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device(atom, DeviceType.PHONE, Brand.LENOVO, "K900", true);
            if (context.consume("IdeaTabA1000-", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.LENOVO, "IdeaTab A100", true);
            if (context.consume("Lenovo A889 Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.LENOVO, "A889", true);

            // Amazon
            if (isKindle(context, true)) {
                if (context.consume("KFTT", MatchingType.BEGINS, MatchingRegion.CONSUMED))
                    return new Device(arm, DeviceType.TABLET, Brand.AMAZON, "Kindle Fire", true);
                if (context.consume("KFJWI", MatchingType.BEGINS, MatchingRegion.CONSUMED))
                    return new Device(arm, DeviceType.TABLET, Brand.AMAZON, "Kindle Fire HD 8.9", true);
                if (context.consume("KFOTE", MatchingType.BEGINS, MatchingRegion.CONSUMED))
                    return new Device(arm, DeviceType.TABLET, Brand.AMAZON, "Kindle Fire (2nd gen)", true);
                if (context.consume("KFOT ", MatchingType.BEGINS, MatchingRegion.CONSUMED))
                    return new Device(arm, DeviceType.TABLET, Brand.AMAZON, "Kindle Fire 7", true);
                if (context.consume("KFTHWI", MatchingType.BEGINS, MatchingRegion.CONSUMED))
                    return new Device(arm, DeviceType.TABLET, Brand.AMAZON, "Kindle Fire HDX 7 (3rd gen)", true);
            }

            // Toshiba
            if (context.consume("IS04", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.TOSHIBA, "Regza IS04", true);
            if (context.consume("AT300 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.TOSHIBA, "AT300", true);
            if (context.consume("AT10-A ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.TOSHIBA, "Excite Pure", true);

            // Other Android stuff
            if (context.consume("MI 3W Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.XIAOMI, "Mi3", true);
            if (context.consume("ADM712HC ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.ODYS, "Neo X7", true);
            if (context.consume("WAX ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.WIKO, "Wax", true);
            if (context.consume("JIMMY ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.WIKO, "Jimmy", true);
            if (context.consume("CINK PEAX ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.WIKO, "Cink Peax", true);
            if (context.consume("CINK FIVE Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.WIKO, "Cink Five", true);
            if (context.consume("Archos 50 Helium 4G ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ARCHOS, "50 Helium", true);
            if (context.consume("Archos 101c Neon Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.ARCHOS, "101c Neon", true);
            if (context.consume("Connect 501 Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.BOULANGER, "Connect 501", true);
            if (context.consume("IM-A850L ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.PANTECH, "Vega R3", true);
            if (context.consume("C351A ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.POSHMOBILE, "Pegasus Plus", true);
            if (context.consume("CUBOT X6 ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.CUBOT, "X6", true);
            if (context.consume("C6750 Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.KYOCERA, "Hydro Elite", true);
            if (context.consume("Event Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.KYOCERA, "Event", true);
            if (context.consume("E500 Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.LOGICOM, "E500", true);
            if (context.consume("PHS-601 Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.CONDOR, "C8", true);
            if (context.consume("A0001 Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ONEPLUS, "One", true);
            if (context.consume("Tabra QAV801 Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.UNKNOWN, "QAV 801", true);
            if (context.consume("ALCATEL ONE TOUCH 7041D Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.ALCATEL, "OneTouch Pop C7", true);
            if (context.consume("M6 Build/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(atom, DeviceType.TABLET, Brand.YUANDAO, "M6", true);
            if (context.consume("M-MP715I ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                return new Device(atom, DeviceType.TABLET, Brand.MEDIACOM, "Smart Pad", true);

            // Generic Android
            if (context.consume("Tablet", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device("", DeviceType.TABLET, Brand.UNKNOWN, "Unknown", true);
        ...
        if (b.getFamily() == BrowserFamily.NETFRONT) {
            ...
            // if (context.consume("(SAMSUNG-)?GT-(S3310/.*", MatchingType.REGEXP, MatchingRegion.REGULAR)) return new
            // Device(arm,DeviceType.PHONE,Brand.SAMSUNG,"S3310");
        }
        ...
        if (o.getFamily() == OSFamily.IOS) {
            ...
            if (context.consume("iPad", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
        ...
        if (o.getVendor() == Brand.RIM) {
            ...
            if (context.consume("BlackBerry9000/", MatchingType.BEGINS, MatchingRegion.CONSUMED))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Bold 9000", true);
            if (context.consume("BlackBerry9300/", MatchingType.BEGINS, MatchingRegion.CONSUMED))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Curve 9300", true);
            if (context.consume("BlackBerry9630/", MatchingType.BEGINS, MatchingRegion.CONSUMED))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Tour 9630", true);
            if (context.consume("BlackBerry9700/", MatchingType.BEGINS, MatchingRegion.CONSUMED))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Bold 9700", true);
            if (context.consume("BlackBerry8520/", MatchingType.BEGINS, MatchingRegion.CONSUMED))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Curve 8520", true);
            if (context.consume("BlackBerry8530/", MatchingType.BEGINS, MatchingRegion.CONSUMED))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Curve 8530", true);
            if (context.consume("BlackBerry 9650", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Bold 9650", true);
            if (context.consume("BlackBerry 9670", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Style 9670", true);
            if (context.consume("BlackBerry 9700", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Bold 9700", true);
            if (context.consume("BlackBerry 9300", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Curve 9300", true);
            if (context.consume("BlackBerry 9790", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Bold 9790", true);
            if (context.consume("BlackBerry 9780", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Bold 9780", true);
            if (context.consume("BlackBerry 9330", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Curve 3G 9330", true);
            if (context.consume("BlackBerry 9320", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Curve 9320", true);
            if (context.consume("BlackBerry 9380", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Curve 9380", true);
            if (context.consume("BlackBerry 9360", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Curve 9360", true);
            if (context.consume("BlackBerry 9930", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Bold 9930", true);
            if (context.consume("BlackBerry 9900", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Bold 9900", true);
            if (context.consume("BlackBerry 9220", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Curve 9220", true);
            if (context.consume("BlackBerry 9800", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Torch 9800", true);
            if (context.consume("BlackBerry 9860", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Torch 9860", true);
            if (context.consume("BlackBerry 9810", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.PHONE, Brand.RIM, "BlackBerry Torch 9810", true);
            if (context.consume("PlayBook", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device(arm, DeviceType.TABLET, Brand.RIM, "PlayBook", true);
            return new Device(arm, DeviceType.UNKNOWN_MOBILE, Brand.RIM, "", true);
        }

        // Motorola
        if (context.consume("MOT-RAZRV3", MatchingType.BEGINS, MatchingRegion.REGULAR))
            return new Device(arm, DeviceType.PHONE, Brand.MOTOROLA, "RAZR v3");
        if (context.consume("MOT-RAZRV6", MatchingType.BEGINS, MatchingRegion.REGULAR))
        ...
        // Consoles
        if (context.consume("Nintendo WiiU", MatchingType.EQUALS, MatchingRegion.CONSUMED))
            return new Device("PowerPC", DeviceType.CONSOLE, Brand.NINTENDO, "Wii U");
        if (context.consume("Nintendo Wii", MatchingType.EQUALS, MatchingRegion.CONSUMED))
            return new Device("PowerPC", DeviceType.CONSOLE, Brand.NINTENDO, "Wii");
        if (context.consume("Nintendo DSi", MatchingType.EQUALS, MatchingRegion.CONSUMED))
            return new Device(arm, DeviceType.CONSOLE, Brand.NINTENDO, "DSi");
        if (context.consume("Super_Nintendo", MatchingType.EQUALS, MatchingRegion.REGULAR))
            return new Device(arm, DeviceType.CONSOLE, Brand.NINTENDO, "Super Nintendo");
        if (context.consume("Nintendo 3DS", MatchingType.EQUALS, MatchingRegion.CONSUMED))
        ...
        if (context.consume("Xbox", MatchingType.EQUALS, MatchingRegion.PARENTHESIS)) {
            if (context.getUA().contains("MSIE 9"))
                return new Device("PowerPC", DeviceType.CONSOLE, Brand.MICROSOFT, "Xbox 360");
            // if (context.getUA().contains("MSIE 7")) return new Device("x86 (PIII)",DeviceType.CONSOLE,Brand.MICROSOFT,"Xbox"); No browser
            // on original Xbox ?
        }
        if (context.consume("PlayStation Vita", MatchingType.BEGINS, MatchingRegion.CONSUMED))
            return new Device(arm, DeviceType.CONSOLE, Brand.SONY, "PlayStation Vita ");
        if (context.consume("PLAYSTATION 3", MatchingType.BEGINS, MatchingRegion.CONSUMED))
        ...
        if (o.getFamily() == OSFamily.WINDOWS_MOBILE || o.getFamily() == OSFamily.WINDOWS_NT) {
            ...
            if (o.getFamily() == OSFamily.WINDOWS_MOBILE)
        ...
        if (o.getFamily() == OSFamily.BADA) {
            if (context.contains("SAMSUNG[ -]GT-.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS) ||
                context.contains("GT-", MatchingType.BEGINS, MatchingRegion.PARENTHESIS)) {
                if (context.consume("(SAMSUNG[ -])?GT-S5380.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Wave Y", true);
                if (context.consume("(SAMSUNG[ -])?GT-S8500.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Wave", true);
                if (context.consume("(SAMSUNG[ -])?GT-S8530.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Wave 2", true);
                if (context.consume("(SAMSUNG[ -])?GT-S5750.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Wave 575", true);
                if (context.consume("(SAMSUNG[ -])?GT-S5253.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Wave 525", true);
                if (context.consume("(SAMSUNG[ -])?GT-S7230.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Wave 723", true);
                if (context.consume("(SAMSUNG[ -])?GT-S8600.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Wave 3", true);
                if (context.consume("(SAMSUNG[ -])?GT-S5780.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Wave 578", true);
                if (context.consume("(SAMSUNG[ -])?GT-S5330.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                    return new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "Wave 533 / Wave 2 Pro", true);
            }
        }
        // Other samsung OS
        if (context.getUA().startsWith("SAMSUNG-GT-") || context.getUA().startsWith("SAMSUNG-SGH-") || context.getUA().startsWith("samsung-gt")
            || context.contains("SPH-M[0-9]{3}", MatchingType.REGEXP, MatchingRegion.BOTH)) {
            Device device = new Device(arm, DeviceType.PHONE, Brand.SAMSUNG, "");
            if (context.consume("SAMSUNG-GT-S5263/", MatchingType.BEGINS, MatchingRegion.REGULAR))
                device.setDevice("Star II");
            if (context.consume("SAMSUNG-GT-S5230/", MatchingType.BEGINS, MatchingRegion.REGULAR))
                device.setDevice("Player One");
            if (context.consume("SPH-M810", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                device.setDevice("Instinct Mini (S30)");
            if (context.consume("SPH-M800", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                device.setDevice("Instinct");
            if (context.consume("SPH-M570", MatchingType.EQUALS, MatchingRegion.REGULAR))
                device.setDevice("Restore");
            if (context.consume("samsung-gt-s5350/", MatchingType.BEGINS, MatchingRegion.REGULAR))
                device.setDevice("Shark");
            if (context.consume("SAMSUNG-SGH-E250/", MatchingType.BEGINS, MatchingRegion.REGULAR))
                device.setDevice("SGH E250");
        ...
        if (o.getFamily() == OSFamily.WINDOWS_NT) {
            ...
            while ((deviceByManufacturer = getWindowsDevice(context)) != null) {
                ...
                if (deviceByManufacturer.getDevice() != null && deviceByManufacturer.getDevice().length() > 0)
                    device = deviceByManufacturer.getDevice();
            }
        ...
        if (o.getFamily() == OSFamily.WINDOWS) {
            String arch = "Intel";
            if (context.consume("Win32", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                arch += " 32 bits";
            if (context.consume("AMD64", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                arch = "AMD 64 bits";
        ...
        if (o.getFamily() == OSFamily.LINUX) {
            ...
            if (context.contains("i686 Linux", MatchingType.BEGINS, MatchingRegion.CONSUMED))
                return new Device("i686", DeviceType.COMPUTER, Brand.UNKNOWN, "");
            if (context.contains("Linux amd64", MatchingType.EQUALS, MatchingRegion.CONSUMED))
                return new Device("AMD 64bits", DeviceType.COMPUTER, Brand.UNKNOWN, "");
            if (context.contains("Linux [0-9.]+-[0-9]-amd64.*", MatchingType.REGEXP, MatchingRegion.CONSUMED))
                return new Device("AMD 64bits", DeviceType.COMPUTER, Brand.UNKNOWN, "");
            if (context.consume("i386", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device("i386", DeviceType.COMPUTER, Brand.UNKNOWN, "");
            if (context.consume("i686", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device("i686", DeviceType.COMPUTER, Brand.UNKNOWN, "");
            if (context.consume("x86_64", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                return new Device("x86_64", DeviceType.COMPUTER, Brand.UNKNOWN, "");
            if (context.contains(".*Linux.*x86_64", MatchingType.REGEXP, MatchingRegion.CONSUMED) ||
                context.consume(".*Linux.*x86_64", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                return new Device("x86_64", DeviceType.COMPUTER, Brand.UNKNOWN, "");
            if (context.contains("Linux", MatchingType.EQUALS, MatchingRegion.CONSUMED) &&
                context.consume("x86_64", MatchingType.EQUALS, MatchingRegion.BOTH))
                return new Device("x86_64", DeviceType.COMPUTER, Brand.UNKNOWN, "");
            if (context.contains("Linux", MatchingType.EQUALS, MatchingRegion.CONSUMED) &&
                context.contains("x86_64", MatchingType.EQUALS, MatchingRegion.CONSUMED))
                return new Device("x86_64", DeviceType.COMPUTER, Brand.UNKNOWN, "");
            if (context.contains(".*Linux.*i686", MatchingType.REGEXP, MatchingRegion.CONSUMED))
                return new Device("i686", DeviceType.COMPUTER, Brand.UNKNOWN, "");
        }
        if (context.contains("IRIX", MatchingType.BEGINS, MatchingRegion.CONSUMED))
            return new Device("MIPS", DeviceType.COMPUTER, Brand.SGI, "IRIX workstation");
        if (o.getFamily() == OSFamily.BEOS) {
            if (context.contains("BeOS BeBox", MatchingType.EQUALS, MatchingRegion.CONSUMED))
                return new Device("PowerPC", DeviceType.COMPUTER, Brand.BE, "BeBox");
        }

        if (o.getFamily() == OSFamily.BSD) {
            if (context.contains("(Free|Open)BSD ([0-9.]+-RELEASE )?i386", MatchingType.REGEXP, MatchingRegion.CONSUMED))
                return new Device("i386", DeviceType.COMPUTER, Brand.UNKNOWN, "");
            if (context.contains("(Free|Open)BSD ([0-9.]+-RELEASE )?amd64", MatchingType.REGEXP, MatchingRegion.CONSUMED))
                return new Device("AMD 64bits", DeviceType.COMPUTER, Brand.UNKNOWN, "");
        }

        if (o.getFamily() == OSFamily.UNIX && o.getDescription().equals("SunOS")) {
            if (context.contains("SunOS.*i86pc.*", MatchingType.REGEXP, MatchingRegion.CONSUMED))
                return new Device("Intel x86", DeviceType.COMPUTER, Brand.UNKNOWN, "PC");
            if (context.contains(".*sun4u", MatchingType.REGEXP, MatchingRegion.CONSUMED))
                return new Device("UltraSPARC", DeviceType.COMPUTER, Brand.SUN, "UltraSPARC");
            if (context.contains(".*sun4[mv]", MatchingType.REGEXP, MatchingRegion.CONSUMED))
    ...
    private static Device getWindowsDevice(UserAgentContext context) {
        if (context.consume("MAAR(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.ACER, "");
        if (context.consume("MDD[CRS](JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.DELL, "");
        if (context.consume("HP([ND]TDF|CMHP)(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.HP, "");
        if (context.consume("(CMN|CPD|CPN)TDF(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.COMPAQ, "");
        if (context.consume("MAAR(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.ACER, "");
        if (context.consume("(MASP|MASA|MASE|MASP)(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.SONY, "");
        if (context.consume("(MASM|SMJB)(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.SAMSUNG, "");
        if (context.consume("(NP06|NP07|NP08|NP09|ASU2|ASJB|MAAU)(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.ASUS, "");
        if (context.consume("(MAFS|FSJB)(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.FUJITSU, "");
        if (context.consume("(MALE|MALN|LCJB|LEN2|MALC)(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.LENOVO, "");
        if (context.consume("MAGW(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.GATEWAY, "");
        if (context.consume("MAMD(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.MEDION, "");
        if (context.consume("MANM(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.HYRICAN, "");
        if (context.consume("(MATM|MATP|TAJB|TNJB|MATB)(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
            return new Device(null, DeviceType.UNKNOWN, Brand.TOSHIBA, "");
        if (context.consume("(MAMI|MAM3)(JS)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
    ...
    static Locale getLocale(String s, UserAgentContext context) {
        if (s.equals("LG")) {
            if (context.contains("LG-", MatchingType.BEGINS, MatchingRegion.BOTH))
                return null;
        }
        ...
        if (s.contains(",")) {
            ...
            for (String l : langs) {
                ...
                switch (lang.length()) {
                    case 0:
                        if (curPos == 0 && langs.length > pos + 1)
                            pos++;
                        break;
                    case 2:
                        ...
                        break;
                    case 5:
                        ok = ok && LocaleHelper.getLanguage(lang.substring(0, 2)) != null && (lang.charAt(2) == '_' || lang.charAt(2) == '-')
                            && LocaleHelper.getCountry(lang.substring(3, 5)) != null;
                        break;
                    default:
                        ok = false;
                        break;
    ...
    static void consumeRandomGarbage(UserAgentContext context, UserAgentDetectionResult result) {
        ...
        if (result.getOperatingSystem().getFamily() == OSFamily.WINDOWS_NT) {
            if (result.getBrowser().getFamily() == BrowserFamily.IE || result.getBrowser().getFamily() == BrowserFamily.OTHER_TRIDENT) {
                context.consume("SLCC1", MatchingType.EQUALS, MatchingRegion.PARENTHESIS); // Some versionning only MS has an explanation
                                                                                           // for
                context.consume("SLCC2", MatchingType.EQUALS, MatchingRegion.PARENTHESIS); // Some versionning only MS has an explanation
                                                                                           // for
            }
            ...
            while (context.consume(".NET4\\.[0-9+][A-Z]?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                ;
            context.ignore("\\+sfgR[0-9a-zA-Z]+(==)?(%3D%3D)?\\+", MatchingType.REGEXP, MatchingRegion.REGULAR); // Haven't got the faintest
                                                                                                                 // idea
            context.ignore("\\{[0-9A-F]{8}\\-[0-9A-F]{4}\\-[0-9A-F]{4}\\-[0-9A-F]{4}\\-[0-9A-F]{12}\\}", MatchingType.REGEXP, MatchingRegion.PARENTHESIS); // Some
                                                                                                                                                           // CLSID
                                                                                                                                                           // or
                                                                                                                                                           // another
                                                                                                                                                           // I
        ...
        if (result.getBrowser().getFamily() == BrowserFamily.OTHER_TRIDENT || result.getBrowser().getFamily() == BrowserFamily.IE) {
            context.ignore("SiteKiosk [0-9\\.]+ Build [0-9]+", MatchingType.REGEXP, MatchingRegion.PARENTHESIS); // Some kiosk public
                                                                                                                 // computer browser
                                                                                                                 // whatever
            ...
            context.ignore("Seekmo [0-9\\.]+", MatchingType.REGEXP, MatchingRegion.PARENTHESIS); // Adware
                                                                                                 // http://www.wiki-security.com/wiki/Parasite/Seekmo/
            context.ignore("iebar", MatchingType.EQUALS, MatchingRegion.PARENTHESIS); // Adware
                                                                                      // http://www.pandasecurity.com/homeusers/security-info/68498/IEBar/
            context.ignore("ShopperReports ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS); // Adware
                                                                                                // http://www.spywareguide.com/spydet_1263_shopperreports.html
            while (context.ignore("IE7-01NET.COM", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                ; // Dunno
            context.ignore("RCP000\\.[0-9]{3}\\.[0-9]{5}/[a-f0-9]+", MatchingType.REGEXP, MatchingRegion.PARENTHESIS); // Dunno
            { // Trojan: http://blog.armorize.com/2010/05/browser-helper-objects-infection-with.html and
              // http://www.spambotsecurity.com/forum/viewtopic.php?f=43&t=1579
            ...
            while (context.ignore("SU [0-9\\.]+", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                ; // Dunno
            while (context.ignore("Sgrunt\\|V[0-9]{3}\\|[0-9]+\\|S-?[0-9]+\\|dial(no)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                ; // Spyware http://forums.spybot.info/showthread.php?39512-Manual-Removal-Guide-for-Sgrunt
            context.ignore("snprtz(\\|[TS][0-9#]+(\\|[0-9#]+Service Pack [0-9#]+)?)?(\\|(dialno|isdn))?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS); // Probably
                                                                                                                                                            // linked
                                                                                                                                                            // to
                                                                                                                                                            // Sgrunt
                                                                                                                                                            // because
                                                                                                                                                            ...
                                                                                                                                                            // logs.
                                                                                                                                                            // Also:
                                                                                                                                                            // http://www.webmasterworld.com/forum11/3136.htm
            context.ignore("ADVPLUGIN\\|K[0-9]{3}\\|[0-9]+\\|S-?[0-9]+\\|dial(no)?", MatchingType.REGEXP, MatchingRegion.PARENTHESIS); // Once
                                                                                                                                       // again,
                                                                                                                                       // correlated
                                                                                                                                       // to
                                                                                                                                       // Sgrunt.
                                                                                                                                       // http://www.yac.mx/en/guides/adware/20141215-how-to-remove-advplugin-pup-by-yac-pc-cleaner.html
            if (context.ignore("EnergyPlugIn", MatchingType.EQUALS, MatchingRegion.PARENTHESIS)) { // Once again, seems correlated to
                                                                                                   // Sgrunt. Also linked to the "dial" part
                                                                                                   // of the UA...
                context.ignore("dial", MatchingType.EQUALS, MatchingRegion.PARENTHESIS);
            }
            context.ignore("E-nrgyPlus", MatchingType.EQUALS, MatchingRegion.PARENTHESIS); // dialer - malware
                                                                                           // http://www.fbmsoftware.com/spyware-net/Application/E-nrgyplus/
            while (context.ignore("PeoplePal [0-9\\.]+", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                ; // Dunno. Doesn't look like a bot. Maybe http://www.paretologic.com/resources/definitions.aspx?remove=PeoplePal+Toolbar ?
                  // This means adware.
            while (context.ignore("IWSS(25|31):[0-9A-Za-z/=\\+]+", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                ; // Dunno, doesn't look like a bot. Maybe Trend Micro InterScan Web Security Suite
            while (context.ignore("IWSS:[0-9A-Za-z\\-/]+", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                ; // Dunno, doesn't look like a bot. Maube Trend Micro InterScan Web Security Suite
            ...
            while (context.ignore("Qwest [0-9\\.]+", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
                ; // Dunno
            while (context.ignore("Qwest Communications", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                ; // Gotta go with corporate installs
            context.ignore("yie6_SBC", MatchingType.EQUALS, MatchingRegion.PARENTHESIS); // IE Optimized for Yahoo. Gotta wonder what the
                                                                                         // fuck this could be...
            ...
            context.ignore("WinNT-PAI [0-9\\.]+", MatchingType.REGEXP, MatchingRegion.PARENTHESIS); // Trojan
            ...
            context.ignore("Badongo [0-9\\.]+", MatchingType.REGEXP, MatchingRegion.PARENTHESIS); // Badongo seems to be an app/add-on for
                                                                                                  // music streaming
            context.ignore("SpamBlockerUtility [0-9\\.]+", MatchingType.REGEXP, MatchingRegion.PARENTHESIS); // Does look like it does more
                                                                                                             // harm that good (Adware
                                                                                                             // application from HotBar)
            while (context.ignore("MS-RTC ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
                ; // NetMeeting
        ...
        if (result.getBrowser().getFamily() == BrowserFamily.OTHER_GECKO || result.getBrowser().getFamily() == BrowserFamily.FIREFOX) {
            context.ignore("ayakawa PGU", MatchingType.EQUALS, MatchingRegion.PARENTHESIS); // http://forums.mozfr.org/viewtopic.php?f=24&t=64837
                                                                                            // seems to indicate this is not a bot
            context.ignore("[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}", MatchingType.REGEXP, MatchingRegion.PARENTHESIS); // No
                                                                                                                                             // idea
            context.ignore("lolifox/", MatchingType.BEGINS, MatchingRegion.REGULAR); // Some bullshit addon to FF
            context.ignore("tete009 .*SSE[2]?.*", MatchingType.REGEXP, MatchingRegion.PARENTHESIS); // Custom FF build
            ...
            context.ignore(";ShopperReports", MatchingType.BEGINS, MatchingRegion.REGULAR); // Adware
                                                                                            // http://www.spywareguide.com/spydet_1263_shopperreports.html
            ...
            context.ignore("BFGod-Speedfox/", MatchingType.BEGINS, MatchingRegion.REGULAR); // Is this a browser? I cannot find anything on
                                                                                            // it anywhere.
            if (context.ignore("Me.dium/", MatchingType.BEGINS, MatchingRegion.REGULAR)) { // Social extension
                                                                                           // http://www.techvibes.com/company-directory/me.dium
            ...
            while (context.ignore("GendFox", MatchingType.EQUALS, MatchingRegion.BOTH))
                ; // This indicates it's probably not a bot : http://forums.mozfr.org/viewtopic.php?t=59568&p=410453
            ...
            context.ignore("UGA6PV", MatchingType.BEGINS, MatchingRegion.REGULAR);// This website would suggest this is operated by a human:
                                                                                  // http://forums.mozfr.org/viewtopic.php?t=66727&p=455964
            ...
            if (context.getUA().indexOf("(CK-") > -1)
                context.ignore("CK-[0-9a-zA-Z\\.\\-_]+", MatchingType.REGEXP, MatchingRegion.PARENTHESIS);// Dunno what that is, but it
                                                                                                          // looks widespread enough not to
                                                                                                          // be a robot
            ...
            while (context.ignoreNextTokens(new Matcher[] {new Matcher("GoogleToolbarFF", MatchingType.EQUALS),
                ...
                MatchingRegion.REGULAR) != null)
                ; // No big deal according to http://forums.mozfr.org/viewtopic.php?t=120841&p=771849
        ...
        // Bullshitware
        while (context.ignore("FunWebProducts", MatchingType.REGEXP, MatchingRegion.BOTH))
            ; // Is this malware? Probably.
        ...
        context.ignore("eSobiSubscriber ", MatchingType.BEGINS, MatchingRegion.PARENTHESIS); // May come from a "offline reading" app or
                                                                                             // from a legit browser on a PC with the app
                                                                                             // installed...
        ...
        context.ignore("AdCentriaIM/", MatchingType.BEGINS, MatchingRegion.REGULAR); // Some king of instant messaging? Couldn't find
                                                                                     // anything on this

        // BullshitToolbarWare
        while (context.ignore("gmx/", MatchingType.BEGINS, MatchingRegion.PARENTHESIS))
            ; // GMX the mail service?
        ...
        context.ignore("YComp [0-9\\.]+", MatchingType.REGEXP, MatchingRegion.PARENTHESIS); // Yahoo Companion (useful I guess if you are
                                                                                            // lonely and in need of a companion). Thanks
                                                                                            // http://www.webmasterworld.com/forum11/1416.htm
        ...
        while (context.ignore("SRS_IT_[0-9A-F]{22}", MatchingType.REGEXP, MatchingRegion.PARENTHESIS))
        ...
        while (context.ignore("Foxy/1", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
            ; // *Maybe* some kind of proxy
        ...
        context.ignore("360SE", MatchingType.EQUALS, MatchingRegion.PARENTHESIS); // 360 Security Explorer
                                                                                  // https://kb.bluecoat.com/index?page=content&id=KB4979&actp=RSS
        context.ignore("Edutice/", MatchingType.BEGINS, MatchingRegion.REGULAR); // French device locking software for education
        while (context.ignore("hotvideobar_[0-9_]+", MatchingType.REGEXP, MatchingRegion.BOTH))
            ; // Malware https://forums.malwarebytes.org/index.php?/topic/26285-hotvideobar/
        while (context.ignore("VB_gameztar", MatchingType.REGEXP, MatchingRegion.BOTH))
            ; // Malware ? GamezTar is.
        ...
        while (consumeEntityFromIEAndFirefox("UGES[VULMYF]?", "[0-9\\.]+", context, result))
            ; // Dunno
        consumeEntityFromIEAndFirefox("LUDI2", null, context, result); // Dunno
        while (consumeEntityFromIEAndFirefox("3P_U(RGD|SEC|VSM|VRM|AMG|ASE|PCPC|ASG)(ES|NL|FR|DE|IT)?", "[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+", context, result))
            ; // Dunno. Usage pattern doesn't point to a bot
        while (consumeEntityFromIEAndFirefox("3P_U(VRM)", "1\\.00\\.1", context, result))
            ; // Dunno. Usage pattern doesn't point to a bot
        consumeEntityFromIEAndFirefox("CK=\\{[0-9a-zA-Z\\+/]+=?=?\\}", null, context, result); // Dunno. Usage pattern doesn't point to a
                                                                                               // bot
        ...
        context.ignore("Google-TR", MatchingType.BEGINS, MatchingRegion.PARENTHESIS); // Looks like an addin, not a bot
                                                                                      // http://commerce.net/deciphering-fluffy-bunny/
        ...
        if (result.getBrowser().getFamily() == BrowserFamily.IE || result.getBrowser().getFamily() == BrowserFamily.OTHER_TRIDENT) {
            ...
            // Other stuff
            while (context.ignore("IEMB3", MatchingType.EQUALS, MatchingRegion.PARENTHESIS))
                ; // Unknown, maybe an ActiveX that is mostly used for nefarious purposes according to
                  // http://forums.3drealms.com/vb/showthread.php?t=32908
     ...
     *
     * @param raw The user agent string as sent by the browser
     * @return The result of the detection
     */
    public UserAgentDetectionResult parseUserAgent(String raw) {
        ...
        UserAgentContext context = new UserAgentContext(raw);
